...
func (ws *WebServer) handleGraph(w http.ResponseWriter, r *http.Request) {
	...
	// Auto-reindex: scan all folders so the graph is always up-to-date
	ws.reindexAllNotes(folderID)

	graph := ws.noteIndex.GetGraph(folderID)
	jsonResponse(w, graph)
}

// reindexAllNotes scans folders and indexes all .md files for the note graph.
// If folderID is empty, all configured folders are scanned.
func (ws *WebServer) reindexAllNotes(folderID string) {
	folders := ws.cfg.GetFolders()
	for _, f := range folders {
		if folderID != "" && f.ID != folderID {
			continue
		}
		absFolder, err := filepath.Abs(f.Path)
		if err != nil {
			continue
		}
		_ = filepath.Walk(absFolder, func(path string, info os.FileInfo, err error) error {
			if err != nil {
				return nil
			}
			if info.IsDir() {
			...
			if strings.ToLower(filepath.Ext(path)) == ".md" {
				...
				ws.noteIndex.IndexNote(f.ID, filepath.ToSlash(rel), content)
			}
			return nil
		})
	}
